...
my %API = (
    send_message         =>  'http://97.64.83.249:3000/openwx/send_message',
);
my $ua = Mojo::UserAgent->new;
if($ARGV[0] eq "-l" or $ARGV[0] eq "-list"){
    my $friend = $ua->get("http://97.64.83.249:3000/openwx/get_friend_info")->res->json;
    print "好友:\n";
    for(@{$friend}){
    ...
    print "=====\n";
    print "群组:\n";
    my $group = $ua->get("http://97.64.83.249:3000/openwx/get_group_info")->res->json;
    for(@{$group}){
        print encode("utf8",$_->{displayname}) || "NULL","\t",$_->{id},"\n";
    }
    exit;
